...
class imagenet100_dataset:
    def __init__(self, trm=None):
        ...
        for c in self.classes:
            self.imgs += glob(f'/disk/scratch_fast/datasets/ImageNet1k/val/{c}/*.JPEG')
...
with torch.no_grad():
    for slot_img, label, img in tqdm(imagenet100_dl):
        pred.append(resnet(img.cuda()).argmax(1).cpu())
        gt.append(label)

imagenet100_map2imagenet1k = {i: imagenet_ds.wnids.index(k) for i, k in enumerate(imagenet100_ds.classes)}
pred = torch.cat(pred)
gt = torch.cat(gt)
gt = torch.tensor([imagenet100_map2imagenet1k[i.item()] for i in gt])
acc = (pred == gt).float().mean().item()
print("Prediction with original images")
print(acc)

new_val_transform = tv.transforms.Compose([
    tv.transforms.Resize(224),
    tv.transforms.ToTensor(),
    tv.transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
])
pred_slots = [8, 16, 32, 64, 128]
class image_tensor_dataset:
    ...
    def __getitem__(self, idx):
        img = self.imgs[idx]
        pil_img = convert_PIL(img)
        v_img = new_val_transform(pil_img)
        return v_img
...
for n_slots in pred_slots:
    ...
    with torch.no_grad():
        ...
        if not os.path.exists(slot_save_path):
            ...
            for slot_img, label, img in tqdm(imagenet100_dl, desc=f"Prediction with {n_slots} slots"):
                ...
                # pred.append(resnet(slot_img).argmax(1).cpu())
                pred_imgs.append(pred_img.cpu())
            pred_imgs = torch.cat(pred_imgs, dim=0)
            os.makedirs(slot_save_path, exist_ok=True)

        pred_ds = image_tensor_dataset(pred_imgs)
        pred_dl = torch.utils.data.DataLoader(pred_ds, batch_size=128, shuffle=False, num_workers=8)

        for pred_img in tqdm(pred_dl, desc=f"Prediction with {n_slots} slots on ResNet"):
            pred.append(resnet(pred_img.cuda()).argmax(1).cpu())

    pred = torch.cat(pred)
    acc = (pred == gt).float().mean().item()
    print(f"Prediction with {n_slots} slots")
    print(acc)
